...
export async function fetchRepoData(owner: string, repo: string): Promise<RepoData> {
  try {
    ...
    const commitsRes = await getOctokit().rest.repos.listCommits({ owner, repo, per_page: 100 });
    ...
    let fileTree: string[] = [];
    try {
      const repoInfo = await getOctokit().rest.repos.get({ owner, repo });
    ...
    try {
      const readmeRes = await getOctokit().rest.repos.getReadme({ owner, repo });
...
export async function fetchArtifactFiles(
  ...
  for (const path of found.slice(0, 5)) {
    try {
      const res = await getOctokit().rest.repos.getContent({ owner, repo, path });
